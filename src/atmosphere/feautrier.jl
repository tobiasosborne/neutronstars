...
"""
function solve_feautrier_all(col::AtmosphereColumn{Tp},
                             μ::AbstractVector{<:Real},
                             w::AbstractVector{<:Real};
                             anisotropic::Bool=true) where {Tp<:Real}
    N = col.N
    M = length(μ)
    nν = col.nν

    # Working type promotes the column's element type with μ, w (typically
    # Float64 from gauss_legendre_half), so AD propagates through P_all.
    R = promote_type(Tp, eltype(μ), eltype(w))

    P_all = zeros(R, N, M, nν)
    J = zeros(R, N, nν)
    f_ν = zeros(R, N, nν)
    h_ν = zeros(R, N, nν)

    for k in 1:nν
        # Find depth range where τ < τ_max (TAU_DIFFUSION). Comparison uses
        # the underlying numerical value; Dual <: Real comparisons work.
        N_eff = N
        for i in 2:N
...
"""
function solve_feautrier_all_adaptive(col::AtmosphereColumn{Tp},
                                       μ::AbstractVector{<:Real},
                                       w::AbstractVector{<:Real};
                                       N_ν::Int=200,
                                       anisotropic::Bool=true) where {Tp<:Real}
    ...
    for k in 1:nν
        ...
        # Create log-spaced per-frequency grid (working in the column's type)
        N_k = min(N_ν, i_max)
        logy_min = log10(max(y_min, Tp(1e-30)))
        logy_max = log10(y_max)
        y_nu = Tp[10.0^logy for logy in range(logy_min, logy_max, length=N_k)]
        ...
        for i in 1:i_max
            J[i, k] = J_common[i]
            f_ν[i, k] = clamp(f_common[i], zero(R), one(R))
            h_ν[i, k] = h_common[i]
        end
...
function _solve_on_grid(N_eff::Int, M::Int,
                        dtau::AbstractVector{Td},
                        μ::AbstractVector{Tm},
                        w::AbstractVector{Tw},
                        ν::Real,
                        T::AbstractVector{TT},
                        ρ_alb::AbstractVector{Tρ},
                        anisotropic::Bool) where {Td<:Real, Tm<:Real, Tw<:Real,
                                                  TT<:Real, Tρ<:Real}
    R = promote_type(Td, Tm, Tw, TT, Tρ, typeof(float(ν)))

    B_tilde = [zeros(R, M, M) for _ in 1:N_eff]
    Q_tilde = [zeros(R, M) for _ in 1:N_eff]
    C_store = [zeros(R, M, M) for _ in 1:N_eff]
...
`::Real` element types are accepted so this composes through ForwardDiff
`Dual` numbers — bracketing uses integer indices (looked up against the
dual's underlying value), interpolation weights and output are Dual-promoted.
"""
function _log_interp(x_old::AbstractVector{<:Real}, y_old::AbstractVector{<:Real},
                      x_new::AbstractVector{<:Real};
                      min_x::Real = eps(Float64))
    @assert all(x -> x >= 0, x_old) "_log_interp requires non-negative x_old"
    n_old = length(x_old)
    n_new = length(x_new)

    # Output type promotes everything that flows into the linear blend.
    R = promote_type(eltype(x_old), eltype(y_old), eltype(x_new),
                     typeof(float(min_x)))
    y_new = zeros(R, n_new)
    logx_old = [log10(max(x, min_x)) for x in x_old]

    for i in 1:n_new
        logx = log10(max(x_new[i], min_x))
        # searchsortedlast on the Vector{R} of log10s works generically on Real.
...
Returns P[i, j] array.
"""
function _solve_single_frequency(col::AtmosphereColumn{Tp}, k::Int,
                                  N_eff::Int,
                                  μ::AbstractVector{Tm},
                                  w::AbstractVector{Tw},
                                  anisotropic::Bool) where {Tp<:Real, Tm<:Real, Tw<:Real}
    M = length(μ)
    ν = col.ν_grid[k]
    R = promote_type(Tp, Tm, Tw)
    ...
    for i in 1:N_eff
        ...
        elseif i == N_eff
            ...
            dt_avg = 0.5 * (dt_prev + dt_next)
            for j in 1:M
                coeff_A = 1 / (dt_prev * dt_avg) * μ[j]^2
                coeff_C = 1 / (dt_next * dt_avg) * μ[j]^2
                A[j, j] = -coeff_A
                C[j, j] = -coeff_C
                B[j, j] = coeff_A + coeff_C + 1
            end
        ...
        # Scattering redistribution + thermal source
        if i > 1 && i < N_eff
            for j in 1:M, jp in 1:M
                if anisotropic
                    cross = 3/16 * (3 + 3*μ[j]^2*μ[jp]^2 - μ[j]^2 - μ[jp]^2)
                else
                    cross = 0.5
                end
                B[j, jp] -= 2 * ρ_alb * cross * w[jp]
            end
            Q .= (1 - ρ_alb) * Bν
        end
